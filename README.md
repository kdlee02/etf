...
- **저장소 포함/제외:** 원본 PDF 코퍼스는 저작권 때문에 `.gitignore` 처리한다(위 4단계로 재생성).
  빌드 산출물인 SQLite 캐시와 FAISS 인덱스는 **배포 대상이라 저장소에 포함**한다 — 클론 후
  `ingest` 없이 바로 실행된다. 갱신하면 diff에 바이너리 변경으로 잡힌다.
